        ...
        while (fgets(line, sizeof(line), fp)) {
            if (size >= capacity) {
                ...
                if (temp == NULL) {
                    free(*points);
                    fclose(fp);
		    return -1; //Memoria berresleitzean errore bat egon balitz, fitxategia ixten du, eta memoria askatzen du.
                }
                *points = temp;
            }

                //irakurritako lineatik float motako bi balio point_t egituran gordetzen dira, t eta angle-en, array-aren size posizioan.
